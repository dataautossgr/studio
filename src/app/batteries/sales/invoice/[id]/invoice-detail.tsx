...
import type { BatterySale, Customer } from '@/lib/data';
...
export default function InvoiceDetail() {
  ...
  const subtotal = sale?.items.reduce((acc, item) => {
      if(item.type === 'scrap') return acc;
      return acc + item.price * item.quantity;
    }, 0) || 0;

  const scrapValue = sale?.items.filter(i => i.type === 'scrap').reduce((acc, item) => acc + item.price * item.quantity, 0) || 0;
        ...
        <div ref={printRef} className={`printable-content print-${receiptSize}`}>
          <div className="receipt-main w-[800px] max-w-full mx-auto border border-black p-5 bg-white shadow-lg print:shadow-none print:border-none">
              ...
              <table className="w-full border-collapse text-sm">
                  ...
                  <tfoot>
                      <tr>
                          <td colSpan={2} rowSpan={4} className="border-none align-bottom">
                              {sale.status !== 'Paid' && (
                       ...
                       <tr>
                          <td className="text-right font-bold pr-2">SCRAP</td>
                          <td className="text-right border border-black p-2 font-mono">{scrapValue.toLocaleString()}</td>
                      </tr>
                      <tr>
